...
namespace DEMO.UI.PlayerSprite
{
    public class HSVColorPicker : MonoBehaviour, IDragHandler
    {
        ...
        {
            float clampedX = Mathf.Clamp(position.x, 0, width);
            float clampedY = Mathf.Clamp(position.y, - height, 1);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            {
                s = Mathf.Clamp(localCursor.x / colorRect.rect.width, 0, 1);
                v = Mathf.Clamp((localCursor.y + colorRect.rect.height) / colorRect.rect.height, 0, 1);
